...
def main(idx):
    ...
    q1 = f"""
    Please answer INITIAL PLANNING 1, with the following instruction. \n
    {MAIN_PROMPT}
    """

    completor = OpenAICompletor()
    ans1 = completor.answer(q1)
    ans2 = completor.answer(q2)
    mkdir('results/test')
    write_txt_file('results/test/ans{}.txt'.format(idx), ans1 + "\n"+ ans2)
    print('finish ', idx)
