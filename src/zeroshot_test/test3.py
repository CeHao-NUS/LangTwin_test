...
def main(idx):
    ...
    The user command is ""place the apple in the closed drawer"".
    """
    ...
    ans1 = """////////// GPT-4 //////////
        ...
        The task requires interaction with an apple and a drawer. The apple will be grasped as a whole, while the bowl will be interacted with at its opening. Let's detect these objects in the environment.

        ```python
        # Detect the apple and the drawer in the environment
        detect_object("apple")
        detect_object("drawer")
        ```
         """
    ...
    Position of drawer: [0.239, 0.285, 0.04]
    Dimensions:
    Width: 0.151
    Length: 0.139
    Height: 0.052
    ...
    print(ans)
    mkdir('results/test_drawer')
    write_txt_file('results/test_drawer/ans{}.txt'.format(idx), ans)
    print('finish ', idx)
